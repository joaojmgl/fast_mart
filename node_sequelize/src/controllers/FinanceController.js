...
module.exports = {
  ...
  async store(req, res) {
    ...
    try {
      ...
      if (existingProduct) {
        ...
        await productController.increaseQuantity(
          req,
          res,
          code,
          quantity,
      ...
      const newProduct = await productController.store(
        product,
        quantity,
        expiry_date,
        company_id
  ...
  async recordSale(req, res) {
    ...
    try {
      ...
      let lastSaleId; // Declara a variável lastSaleId
    ...
    try {
      ...
      if (!startDate || !endDate || !filterType) {
        return res.status(400).send({
          status: 0,
          message:
            "Data de início, data final e tipo de filtragem são obrigatórios.",
        });
      }
      ...
      if (!results || results.length === 0) {
        return res.status(404).send({
          status: 0,
          message:
            "Nenhum registro encontrado para o intervalo de datas especificado.",
        });
      }
  ...
  async cancelSale(req, res) {
    ...
    try {
      ...
      if (!financeRecord) {
        return res
          .status(404)
          .json({ message: "Registro de venda não encontrado" });
      }
      ...
      return res
        .status(500)
        .json({ message: `Erro ao cancelar a venda: ${error.message}` });
    }
  },
};
